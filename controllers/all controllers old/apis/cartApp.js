 ...
 */
function getProductName(pid, callback){
    ...
    if(req.body.device_token !== ''){ 
        Cart.find({ user_id: req.body.user_id}, function(error, cartRes) {
                ...
                async.parallel(
                    [
                        function(callback){// To get product name
                            getProductName(CproductRes.product_id, function(err, pres){
                                if(pres.length){
                                    pArr.name = pres,name;
                                    pArr.sku = pres.sku;
            ...
            function(err){
              ...
              return res.json({"status":'success',"msg":'Fetch all products.',data:tempCartProduct});
            });
        });
    }else{
